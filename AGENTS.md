...
- Treat related multi-step work as one package: approve one package plan, keep one task branch and implementer across its slices, and resume that implementer with the existing plan for continuations and fixes. Do not restart the pipeline for each slice. The implementer runs relevant step checks as work progresses; invoke the final test-runner and both independent reviewers only after the complete package is committed and its diff is frozen.
- Use an intermediate independent review gate only when the plan explicitly identifies a separately reviewable critical boundary involving payments, security, authentication, persistence, schema, migrations, or an external API contract. It does not replace the mandatory final verification and reviews. Otherwise, repeat independent reviews only after blocking fixes.
